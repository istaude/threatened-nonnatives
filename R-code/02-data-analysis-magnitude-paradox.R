...
2513 / (6682 + 2513)
# 9195 non-native species (excluding hybrids, and accounted for newest WCVP taxonomy)
d_final_onlythreatened %>% select(taxon_name) %>% distinct %>% nrow


# how many non-natives are also red-listed, including near-threatened sp
d_final_all %>%
  group_by(taxon_name) %>%
  summarize(status_combined = paste(sort(unique(status)), collapse = " and ")) %>% 
  count(status_combined)
...
2862 / (6333 + 2862)
# 9195 non-native species (excluding hybrids, and accounted for newest WCVP taxonomy)
d_final_all %>% select(taxon_name) %>% distinct %>% nrow
...
ggplot(data, aes(x = 2, y = value, fill = category)) +
  ...
  xlim(0.5, 2.5) + 
  labs(fill = "") +  
  scale_fill_manual(values = c("#1D6774", "#FAC55F")) + 
  ...
  annotate("text", x = 1.5, y = 0, 
           label = "Naturalized species\nthreatened in parts of\ntheir native ranges", 
           color = "#95846d", size = 4.5,lineheight = 0.8,
           hjust = 0.5, vjust = 2,
           family = "Arial Narrow") +
  annotate("text", x = 1.5, y = 0, 
           label = "N = 9,195\nspecies assessed", 
           color = "#95846d", size = 4, lineheight = 0.8,
           hjust = 0.5, vjust = 6,
           family = "Arial Narrow")
...
# i.e., using only data from glonaf and rls when data for both is available
# in a country
d_countrysame %>%
  group_by(taxon_name) %>%
  summarize(status_combined = paste(sort(unique(status)), collapse = " and ")) %>% 
...
# test whether the range size of all species in wcvp predicts rl listing prob
# on a national RL
...
# all species in wcvp, exclude hybrids, apomictic etc...
# for apomictic genera repeat
